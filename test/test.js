...
describe('Automated tests', function () {
  ...
  describe('`addToCollection` pushes record object into the array & returns the object', function () {
    it('`addToCollection` pushes record object into the array & returns the object', function () {
      ...
      expect(
        albumCountAfterAdd - albumCountBeforeAdd,
        'Expected one value to be added to the `primesCollection` array'
      ).to.equal(1);
      expect(result, 'Expected the function to return a value').to.exist;
      expect(
        primesCollection.at(-1),
        'Expected returned album object to match the final object in the `primesCollection` array'
      ).to.equal(result);
    });
  });
  ...
  describe('`showCollection` takes in an array, loops over it, correctly logs each item', function () {
    it('`showCollection` takes in an array, loops over it, correctly logs each item', function () {
      ...
      // or false for this.)
      expect(
        testEachLogStatement(myCollection, logs),
        'For each album, expected `title`, `artist`, and `yearPublished` within the logged output'
      ).to.equal(true);
  ...
  describe('`findByArtist` takes in a collection and artist, then returns an array of matching albums', function () {
    it('`findByArtist` takes in a collection and artist, then returns an array of matching albums', function () {
      ...
      expect(
        findByArtist(primesCollection, 'Beyonce'),
        'Expect an array to be returned'
      ).to.be.an.instanceof(Array);
      expect(
        findByArtist(primesCollection, 'Beyonce'),
        'Expect an empty array in the case of no match'
      ).to.be.an('array').that.is.empty;
      expect(
        findByArtist(primesCollection, 'Cécile McLorin Salvant'),
        'Expect one matching result to be returned'
      ).to.deep.equal(cecileResults);
      expect(
        findByArtist(primesCollection, 'Hiroshi Yoshimura'),
        'Expect two matching results to be returned'
      ).to.deep.equal(hiroshiResults);
    });
  });
  ...
  describe('STRETCH: `search` takes in a criteria object and returns an array of matching albums', function () {
    it('STRETCH: `search` takes in a criteria object and returns an array of matching albums', function () {
      ...
      if (search === undefined) {
        ...
        expect(
          search(primesCollection),
          'Expect entire collection without search object'
        ).to.deep.equal(primesCollection);
        expect(
          search(primesCollection, {}),
          'Expect entire collection with empty search object'
        ).to.deep.equal(primesCollection);
        expect(
          search(primesCollection, { artist: '', year: 2023 }),
          'Expect entire collection with empty artist string'
        ).to.deep.equal(primesCollection);
        expect(
          search(primesCollection, { artist: 'Wilco', year: 1908 }),
          'Expect an empty array when searching for a Wilco album from 1908'
        ).to.be.an('array').that.is.empty;
        expect(
          search(primesCollection, { artist: 'Wilco', year: 2002 })
        ).to.deep.equal(wilcoResults);
        expect(
          search(primesCollection, { artist: 'Hiroshi Yoshimura', year: 1986 })
        ).to.deep.equal(hiroshiResults);
      }
    });
  });
